...
func (self *_runtime) evaluateConditionalExpression(node *parser.ConditionalExpression) Value {
...
func (self *_runtime) evaluateVariableExpression(node *parser.VariableExpression) Value {
...
func (self *_runtime) evaluateNew(node *parser.NewExpression) Value {
...
func (self *_runtime) evaluateArray(node *parser.ArrayLiteral) Value {
...
func (self *_runtime) evaluateObject(node *parser.ObjectLiteral) Value {
	...
	for _, property := range node.Value {
		...
		case "get":
			getter := self.newNodeFunction(property.Value.(*parser.FunctionExpression), self.LexicalEnvironment())
		...
		case "set":
			setter := self.newNodeFunction(property.Value.(*parser.FunctionExpression), self.LexicalEnvironment())
			descriptor := _property{}
			descriptor.mode = 0211
...
func (self *_runtime) evaluateRegExpLiteral(node *parser.RegExpLiteral) Value {
	return toValue_object(self._newRegExp(node.Pattern, node.Flags))
}

func (self *_runtime) evaluateUnaryExpression(node *parser.UnaryExpression) Value {
...
func (self *_runtime) evaluateAssignExpression(node *parser.AssignExpression) Value {
...
func (self *_runtime) evaluateComparison(node *parser.BinaryExpression) Value {
...
func (self *_runtime) evaluateBinaryExpression(node *parser.BinaryExpression) Value {
...
func (self *_runtime) evaluateCall(node *parser.CallExpression, withArgumentList []interface{}) Value {
	callee := self.evaluate(node.Callee)
	calleeValue := self.GetValue(callee)
	argumentList := []Value{}
...
func (self *_runtime) evaluateFunction(node *parser.FunctionExpression) Value {
	return toValue_object(self.newNodeFunction(node, self.LexicalEnvironment()))
}

func (self *_runtime) evaluateDotExpression(node *parser.DotExpression) Value {
	target := self.evaluate(node.Left)
	targetValue := self.GetValue(target)
	// TODO Pass in base value as-is, and defer toObject till later?
	return toValue(newPropertyReference(self.toObject(targetValue), node.Identifier.Name, false, node))
}

func (self *_runtime) evaluateBracketExpression(node *parser.BracketExpression) Value {
	target := self.evaluate(node.Left)
	targetValue := self.GetValue(target)
...
func (self *_runtime) evaluateIdentifier(node *parser.Identifier) Value {
	name := node.Name
...
func (self *_runtime) evaluateSequenceExpression(node *parser.SequenceExpression) Value {
	var result Value
