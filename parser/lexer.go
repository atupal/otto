...
func (self *_parser) scanIdentifier() (string, error) {
	...
	literal := string(self.source[offset:self.chrOffset])
	if parse {
		return parseStringLiteral(literal), nil
	}
	return literal, nil
}
...
func (self *_parser) scan() (tkn token.Token, literal string, idx Idx) {
	...
	for {
		...
		case isIdentifierStart(chr):
			...
			if len(literal) > 1 {
				...
				tkn = token.Keyword(literal)
				if tkn > 0 {
					switch tkn {
					case token.THROW, token.RETURN, token.BREAK, token.CONTINUE:
...
func (self *_parser) chrAt(index int) _chr {
	value, width := utf8.DecodeRuneInString(self.source[index:])
	return _chr{
		value: value,
		width: width,
	}
...
func (self *_parser) _peek() rune {
	if self.offset+1 < self.length {
		return rune(self.source[self.offset+1])
	}
	return -1
}
...
func (self *_parser) read() {
	if self.offset < self.length {
		...
		chr, width := rune(self.source[self.offset]), 1
		switch {
		case chr >= utf8.RuneSelf:
			// Not ASCII
			chr, width = utf8.DecodeRuneInString(self.source[self.offset:])
			if chr == utf8.RuneError && width == 1 {
				self.error(0, "RuneError")
				// FIXME Byte order mark?
			}
		}
		self.offset += width
		self.chr = chr
...
func (self *_parser) scanEscape(quote rune) {
	...
	case 'x':
		self.read()
		length, base, limit = 2, 16, 255
	case 'u':
		self.read()
		length, base, limit = 4, 16, unicode.MaxRune
	case 'U':
		self.read()
		length, base, limit = 8, 16, unicode.MaxRune
	default:
	...
	for ; length > 0 && self.chr != quote && self.chr >= 0; length-- {
		...
		if digit >= base {
			self.error(offset, "Illegal character in escape sequence")
			break
		}
		value = value*base + digit
		self.read()
	}
	// In case of an error, consume remaining characters
	for ; length > 0 && self.chr != quote && self.chr >= 0; length-- {
		self.read()
	}
	if value > limit || 0xD800 <= value && value < 0xE000 {
		self.error(offset, "Escape sequence is invalid Unicode code point")
	}
}

func (self *_parser) scanString(offset int) (string, error) {
	// " ' /
	quote := rune(self.source[offset])
	...
	return string(self.source[offset:self.chrOffset]), nil
	...
	for len(str) > 0 {
		...
		if chr >= utf8.RuneSelf {
			...
			default:
				if chr >= utf8.RuneSelf {
					// FIXME
					panic("SyntaxError: parseStringLiteral")
				}
				value = rune(chr)
			}
		}
	...
	if self.chr == '0' {
		...
		if self.chr == 'x' || self.chr == 'X' {
			...
			if isDigit(self.chr, 16) {
				...
				return token.ILLEGAL, self.source[offset:self.chrOffset]
			}
...
octal:
	...
	return tkn, self.source[offset:self.chrOffset]
}
