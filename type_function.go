...
func (runtime *_runtime) newNodeFunctionObject(node *parser.FunctionExpression, scopeEnvironment _environment) *_object {
	self := runtime.newClassObject("Function")
	self.value = _functionObject{
...
type _nodeCallFunction struct {
	node             *parser.FunctionExpression
	scopeEnvironment _environment // Can be either Lexical or Variable
}

func newNodeCallFunction(node *parser.FunctionExpression, scopeEnvironment _environment) *_nodeCallFunction {
